...
- **Assistant Settings** has two tabs:
  - **Character**: the Desktop Assistant on/off switch, the character grid, Size, and the speech Balloon style (Classic, Aqua, macOS, Windows 98, System 7).
    **Add…** at the end of the grid makes a character from a generated sprite sheet (**copy the prompt** gives the image-model prompt): pick the image, name it (and optionally say who it is, which the model is told so the character knows itself), and it is added and selected. A custom character can be renamed (click its name), described, or deleted.
  - **Behavior**: Greeting, Speech, Response Style (Concise, Normal, Chatty, Detailed, Friendly, Professional, Playful), local Model, the Focus timer (focus and break length in hours, minutes and seconds; the end Sound), Claude Code (below), and free-form Instructions (up to 500 characters).
- **Claude Code ▸ Work along** (Behavior tab), like Codex's pets: while a Claude Code session works, the pet works too (its thinking animation), and its balloon shows what needs you: Claude waiting for a permission or an answer, a turn that stopped with an error, or one of 20 seconds or more that is done (with the first line of Claude's reply; quicker ones you watched finish). The most urgent one shows, with a count of the rest; each goes away once seen (you answer, prompt that session again, or close the balloon), and clicking it brings the terminal forward. Notices are in Chinese when you write to Claude in Chinese, and the pet remembers them, so you can ask it what Claude did. The switch adds hooks to `~/.claude/settings.json` (`$CLAUDE_CONFIG_DIR` if set; backed up to `settings.json.deskling-backup` first, permissions and symlinks kept; hooks from an older Deskling are updated at launch) that post each event to Deskling on `127.0.0.1:47823`; they stay silent when Deskling isn't running, and turning the switch off removes them. `npx electron . --selftest --edge --hold=60000 --claude-code[=<port>]` listens without installing them, on another port if your own Deskling is running (QA).
